#' @title STD plots
#' @description
#' Function to make plots of STD samples to screen peak quality and retention
#' time shifts.
#'
  ...
  function(compound_info,
    ...
    if (length(grep(STD_pattern, sample_names)) != 0) {
      ...
      if (dir.exists(paste0(
        output_directory, "Diagnostic_STDs_Batch_",
        batchnr
      )) == FALSE) {
        dir.create(paste0(output_directory, "Diagnostic_STDs_Batch_", batchnr))
      }
